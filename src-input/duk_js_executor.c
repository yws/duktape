...
/* FIXME: rework... */
DUK_LOCAL void duk__reconfig_valstack_ecma_catcher(duk_hthread *thr, duk_activation *act, duk_catcher *cat) {
	duk_hcompfunc *h_func;
	duk_idx_t clamp_top;
...
DUK_LOCAL void duk__set_catcher_regs(duk_hthread *thr, duk_catcher *cat, duk_tval *tv_val_unstable, duk_small_uint_t lj_type) {
	duk_tval *tv1;
...
DUK_LOCAL void duk__handle_catch(duk_hthread *thr, duk_tval *tv_val_unstable, duk_small_uint_t lj_type) {
	...
	duk__set_catcher_regs(thr, cat, tv_val_unstable, lj_type);
	...
	act = thr->callstack_curr;
	DUK_ASSERT(act != NULL);
	duk__reconfig_valstack_ecma_catcher(thr, act, cat);
 ...
 check_longjmp:
	...
	case DUK_LJ_TYPE_THROW: {
		...
		while (thr->callstack_curr != NULL) {
			...
			/* FIXME: make this efficient.. problem is, value stack is unconfigured
			 * and we must never ever throw!
			 */
...
DUK_LOCAL duk_small_uint_t duk__handle_return(duk_hthread *thr,
	...
	if (thr->callstack_top >= 2) {
		...
		/* FIXME: explicit unwind not necessary */
#if 0
...
#define DUK__RETURN_SHARED() do { \
		...
		case DUK_OP_TRYCATCH: {
			...
			if (a & DUK_BC_TRYCATCH_FLAG_CATCH_BINDING) {
				 ...
				 */
				cat = act->cat;  /* FIXME: relookup not needed */
		...
		case DUK_OP_ENDFIN: {
			...
			DUK_ASSERT(DUK_TVAL_IS_NUMBER(tv1));
			cont_type = (duk_small_uint_t) DUK_TVAL_GET_NUMBER(tv1);  /* FIXME: fastint */
